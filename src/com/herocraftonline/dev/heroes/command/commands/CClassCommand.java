	...
	public void execute(CommandSender sender, String[] args) {
		if(sender instanceof Player){
			if(Properties.validateClass(args[0]) == ""){
				Misc.sendMessage((Player) sender, "Sorry, that isn't a class!");
				return;
			}
			String cClass = Properties.validateClass(args[0]);
			if(Properties.primaryClass(cClass)){
				if(player.getClass((Player) sender) == "Vagrant"){
					player.setClass((Player) sender, cClass);
					Misc.sendMessage((Player) sender, "Welcome to the path of the " + cClass);
				}else{
					changeClass((Player) sender, cClass);
					Misc.sendMessage((Player) sender, "Welcome to the path of the " + cClass);
				}
			}else{
				Misc.sendMessage((Player) sender, "Sorry " + cClass + " isn't a primary class!");
			}
		}
	}
